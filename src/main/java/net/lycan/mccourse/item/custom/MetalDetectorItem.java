...
public class MetalDetectorItem extends Item {
        ...
        if(!context.getWorld().isClient) {
            ...
            for (int i = 0; i <= positionClicked.getY() + 64; i++ ) {
                ...
                if(isValuableBlock(blockState)){
                    outPutValuableCoordianates(positionClicked.down(i), player, block);
                    foundBlock = true;
    ...
    private void outPutValuableCoordianates(BlockPos position, PlayerEntity player, Block block) {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(Screen.hasShiftDown()){
            tooltip.add(Text.translatable("tooltip.mccourse.metaldetector.shift"));
        }else{
            tooltip.add(Text.translatable("tooltip.mccourse.metaldetector"));
        }
    }
}
